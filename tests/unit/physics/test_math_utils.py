...
from physics.math_utils import quat_multiply, quat_rotate, quat_to_rotmat, quat_to_rotmat_np
...
class TestQuaternionOperations:
  ...
  def test_quat_to_rotmat_identity(self):
    ...
    q = Tensor([[1.0, 0.0, 0.0, 0.0]], dtype=np.float32)
    R = quat_to_rotmat(q)
  ...
  def test_quat_to_rotmat_known_rotation(self):
    ...
    angle = np.pi / 2
    q = Tensor([[np.cos(angle/2), 0.0, 0.0, np.sin(angle/2)]], dtype=np.float32)
    R = quat_to_rotmat(q)
  ...
  def test_quat_multiply(self):
    ...
    angle = np.pi / 2
    q1 = Tensor([[np.cos(angle/2), 0.0, 0.0, np.sin(angle/2)]], dtype=np.float32)
    q2 = q1.clone()
  ...
  def test_quat_rotate(self):
    ...
    v = Tensor([[1.0, 0.0, 0.0]], dtype=np.float32)
  ...
  def test_quat_to_rotmat_consistency(self):
    ...
    R_numpy = quat_to_rotmat_np(q_np)
